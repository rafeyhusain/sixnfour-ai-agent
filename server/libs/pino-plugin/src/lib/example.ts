...
const logger = Logger('PinoPlugin-Example');
/**
...
export function example6ErrorWithoutStackTrace(): void {
  ...
  try {
    ...
    authService.error('Authentication configuration failed', error);
  }
}
...
export function example9StructuredLogging(): void {
  ...
  logger.failed2('Authentication error context', errorContext);
}
...
export function example12LogFileDemonstration(): void {
  ...
  fileLogger.error('Error message in log file', new Error('Test error'));
}
