import { LlmCore, LlmRequest, LlmResult, LlmError } from './llm-core';
...
export class LlmPlugin {
  ...
  askSample(request: LlmRequest): LlmResult {
    return {
      content: `[
  {
    "id": "{id}",
    "caption": "{caption} 🌴🎉",
    "body": "Join us for a fresh Bahamian brunch, featuring conch fritters, guava duff, and more. Our chefs have crafted dishes to make your taste buds dance. 🍳👏",
    "hashtags": ["NYDBrunch", "BahamianCuisine", "SparklingCocktails"],
    "mediaUrls": ["https://ik.imagekit.io/seawing/content-wing/assets/resturant/photo.mp4",
      "https://ik.imagekit.io/seawing/content-wing/assets/resturant/video.mp4"
      ]
  }
]`,
      ...
      metadata: {
        totalDuration: 25565764300,
        loadDuration: 74583300,
        promptEvalDuration: 374432500,
        evalDuration: 25114644800,
        done: true
      }
    };
  }

  async ask(request: LlmRequest, sample: boolean = false): Promise<LlmResult> {
    try {
      if (sample) {
        return this.askSample(request);
      }
  ...
  async askWithRetry(request: LlmRequest, maxRetries: number = 3): Promise<LlmResult> {
