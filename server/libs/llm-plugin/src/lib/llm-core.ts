...
import { PinoPlugin, Logger } from '@awing/pino-plugin'
...
export abstract class LlmCore {
  ...
  protected async makeRequest<T>(
    ...
    if (!this.config.enabled) {
      throw new Error(`Skipped [${this.config.name}] call — disabled in settings`);
    }
  ...
  protected logResponse(response: LlmResult, provider: string, duration: number): void {
  ...
  abstract ask(request: LlmRequest): Promise<LlmResult>;
} 